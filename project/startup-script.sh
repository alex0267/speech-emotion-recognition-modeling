...
REMOTE_IMAGE_NAME=$(curl http://metadata/computeMetadata/v1/instance/attributes/remote_image_name -H "Metadata-Flavor: Google")
...
# [END startup_script]
#pull remote image
docker pull eu.gcr.io/$PROJECT_ID/$REMOTE_IMAGE_NAME:latest
#bash in that image
#docker run --rm -i -t $REMOTE_IMAGE_NAME bash